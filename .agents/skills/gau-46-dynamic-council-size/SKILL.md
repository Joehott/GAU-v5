---
name: gau-46-dynamic-council-size
description: Ajustar quantidade e especialidades conforme lacunas e ganho marginal. Integra a ideia 46 do GAU v5.
---
...
- **Gatilho**: Ajustar quantidade e especialidades conforme lacunas e ganho marginal.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `dynamic-size`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/dynamic-size.json`.
   - Registre: `mission`, `idea_id` (46), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `dynamic-size.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
