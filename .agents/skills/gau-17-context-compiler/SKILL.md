---
name: gau-17-context-compiler
description: Preparar recorte de contexto relevante e limitado para cada agente. Integra a ideia 17 do GAU v5.
---
...
- **Gatilho**: Preparar recorte de contexto relevante e limitado para cada agente.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `context-packet`.
...
2. **Apoio Executável**:
   - Utilize o comando de apoio: `python .gau/runtime/gau.py --project . context`.
   - Utilize os modelos JSON de `.gau/examples/` correspondentes.
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/context-packet.json`.
   - Registre: `mission`, `idea_id` (17), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `context-packet.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
