---
name: gau-59-novelty-detector
description: Comparar tentativa nova com histórico e exigir diferença substancial. Integra a ideia 59 do GAU v5.
---
...
- **Gatilho**: Comparar tentativa nova com histórico e exigir diferença substancial.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `novelty`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/novelty.json`.
   - Registre: `mission`, `idea_id` (59), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `novelty.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
