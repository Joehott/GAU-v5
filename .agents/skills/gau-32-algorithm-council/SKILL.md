---
name: gau-32-algorithm-council
description: Comparar correção, complexidade e contraexemplos em diferentes escalas. Integra a ideia 32 do GAU v5.
---
...
- **Gatilho**: Comparar correção, complexidade e contraexemplos em diferentes escalas.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `algorithm`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/algorithm.json`.
   - Registre: `mission`, `idea_id` (32), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `algorithm.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
