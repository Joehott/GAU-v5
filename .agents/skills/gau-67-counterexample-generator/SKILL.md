---
name: gau-67-counterexample-generator
description: Reproduzir entrada concreta que possa refutar hipótese, invariante ou algoritmo. Integra a ideia 67 do GAU v5.
---
...
- **Gatilho**: Reproduzir entrada concreta que possa refutar hipótese, invariante ou algoritmo.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `counterexamples`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/counterexamples.json`.
   - Registre: `mission`, `idea_id` (67), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `counterexamples.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
