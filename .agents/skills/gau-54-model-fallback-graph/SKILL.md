---
name: gau-54-model-fallback-graph
description: Reordenar alternativas disponíveis depois de falha de modelo. Integra a ideia 54 do GAU v5.
---

# Model Fallback Graph

## 1. Entrada e Ativação

- **Gatilho**: Reordenar alternativas disponíveis depois de falha de modelo.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `model-fallback`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/model-fallback.json`.
   - Registre: `mission`, `idea_id` (54), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `model-fallback.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
