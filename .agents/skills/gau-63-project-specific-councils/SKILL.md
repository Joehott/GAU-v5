---
name: gau-63-project-specific-councils
description: Ajustar composição ao projeto usando experiências comprovadas. Integra a ideia 63 do GAU v5.
---
...
- **Gatilho**: Ajustar composição ao projeto usando experiências comprovadas.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `project-councils`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/project-councils.json`.
   - Registre: `mission`, `idea_id` (63), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `project-councils.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
