---
name: gau-35-regression-hunters
description: Selecionar testes de regressão por diff, contratos e histórico de fragilidade. Integra a ideia 35 do GAU v5.
---
...
- **Gatilho**: Selecionar testes de regressão por diff, contratos e histórico de fragilidade.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `regression`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/regression.json`.
   - Registre: `mission`, `idea_id` (35), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `regression.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
