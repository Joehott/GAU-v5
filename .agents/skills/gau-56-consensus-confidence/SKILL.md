---
name: gau-56-consensus-confidence
description: Avaliar correlação e evidência sem inventar probabilidade calibrada. Integra a ideia 56 do GAU v5.
---
...
- **Gatilho**: Avaliar correlação e evidência sem inventar probabilidade calibrada.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `consensus-confidence`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/consensus-confidence.json`.
   - Registre: `mission`, `idea_id` (56), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `consensus-confidence.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
