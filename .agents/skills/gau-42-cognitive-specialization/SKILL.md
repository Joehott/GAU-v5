---
name: gau-42-cognitive-specialization
description: Escolher perspectivas distintas e substituir perfis redundantes. Integra a ideia 42 do GAU v5.
---
...
- **Gatilho**: Escolher perspectivas distintas e substituir perfis redundantes.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `cognitive-profiles`.
...
2. **Apoio Executável**:
   - Utilize o comando de apoio: `python .gau/runtime/gau.py --project . record --kind council`.
   - Utilize os modelos JSON de `.gau/examples/` correspondentes.
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/cognitive-profiles.json`.
   - Registre: `mission`, `idea_id` (42), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `cognitive-profiles.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
