---
name: gau-09-verification-swarm
description: Verificar requisitos, execução, regressões, segurança, desempenho e integração. Integra a ideia 9 do GAU v5.
---
...
- **Gatilho**: Verificar requisitos, execução, regressões, segurança, desempenho e integração.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `verification`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/verification.json`.
   - Registre: `mission`, `idea_id` (9), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `verification.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
