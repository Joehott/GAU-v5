---
name: gau-12-model-elo-ranking
description: Registrar duelos comparáveis entre versões de modelos por domínio. Integra a ideia 12 do GAU v5.
---
...
- **Gatilho**: Registrar duelos comparáveis entre versões de modelos por domínio.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `model-elo`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/model-elo.json`.
   - Registre: `mission`, `idea_id` (12), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `model-elo.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
