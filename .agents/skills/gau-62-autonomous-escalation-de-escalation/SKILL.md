---
name: gau-62-autonomous-escalation-de-escalation
description: Ampliar ou reduzir equipe após convergência, divergência ou bloqueio. Integra a ideia 62 do GAU v5.
---

# Autonomous Escalation / De-escalation

## 1. Entrada e Ativação

- **Gatilho**: Ampliar ou reduzir equipe após convergência, divergência ou bloqueio.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `escalation`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/escalation.json`.
   - Registre: `mission`, `idea_id` (62), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `escalation.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
