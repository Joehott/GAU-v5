---
name: gau-68-proof-obligation-system
description: Registrar obrigação crítica e só resolvê-la com evidência atual. Integra a ideia 68 do GAU v5.
---
...
- **Gatilho**: Registrar obrigação crítica e só resolvê-la com evidência atual.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `proof-obligations`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/proof-obligations.json`.
   - Registre: `mission`, `idea_id` (68), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `proof-obligations.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
