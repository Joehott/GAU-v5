---
name: gau-24-research-swarm
description: Pesquisar fontes externas quando a tarefa depende de informação atual. Integra a ideia 24 do GAU v5.
---
...
- **Gatilho**: Pesquisar fontes externas quando a tarefa depende de informação atual.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `research`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/research.json`.
   - Registre: `mission`, `idea_id` (24), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `research.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
