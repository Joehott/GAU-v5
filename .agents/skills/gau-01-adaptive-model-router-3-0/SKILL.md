---
name: gau-01-adaptive-model-router-3-0
description: Escolher equipe/modelo no início ou após falha. Integra a ideia 1 do GAU v5.
---

# Adaptive Model Router 3.0

## 1. Entrada e Ativação

- **Gatilho**: Escolher equipe/modelo no início ou após falha.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `routing`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/routing.json`.
   - Registre: `mission`, `idea_id` (1), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `routing.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
