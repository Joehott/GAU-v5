---
name: gau-60-minority-preservation
description: Preservar objeção minoritária sustentada até teste discriminatório. Integra a ideia 60 do GAU v5.
---
...
- **Gatilho**: Preservar objeção minoritária sustentada até teste discriminatório.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `minority`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/minority.json`.
   - Registre: `mission`, `idea_id` (60), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `minority.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
