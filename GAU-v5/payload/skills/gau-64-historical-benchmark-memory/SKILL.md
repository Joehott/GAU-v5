---
name: gau-64-historical-benchmark-memory
description: Guardar resultados reais comparáveis com versões, domínio e condições. Integra a ideia 64 do GAU v5.
---

# Historical Benchmark Memory

## 1. Entrada e Ativação

- **Gatilho**: Guardar resultados reais comparáveis com versões, domínio e condições.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `benchmark-history`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/benchmark-history.json`.
   - Registre: `mission`, `idea_id` (64), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `benchmark-history.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
