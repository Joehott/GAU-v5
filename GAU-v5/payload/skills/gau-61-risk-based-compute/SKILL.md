---
name: gau-61-risk-based-compute
description: Dimensionar esforço pelo impacto do erro e reversibilidade. Integra a ideia 61 do GAU v5.
---
...
- **Gatilho**: Dimensionar esforço pelo impacto do erro e reversibilidade.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `risk-compute`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/risk-compute.json`.
   - Registre: `mission`, `idea_id` (61), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `risk-compute.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
