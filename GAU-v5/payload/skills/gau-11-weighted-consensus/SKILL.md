---
name: gau-11-weighted-consensus
description: Priorizar hipóteses usando evidência e pesos sem votação cega. Integra a ideia 11 do GAU v5.
---
...
- **Gatilho**: Priorizar hipóteses usando evidência e pesos sem votação cega.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `weighted-consensus`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/weighted-consensus.json`.
   - Registre: `mission`, `idea_id` (11), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `weighted-consensus.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
