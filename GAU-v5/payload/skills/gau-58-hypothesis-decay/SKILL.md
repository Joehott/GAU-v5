---
name: gau-58-hypothesis-decay
description: Reduzir prioridade de hipótese sem novo suporte sem tratá-la como refutada. Integra a ideia 58 do GAU v5.
---
...
- **Gatilho**: Reduzir prioridade de hipótese sem novo suporte sem tratá-la como refutada.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `hypothesis-decay`.
...
2. **Apoio Executável**:
   - Utilize o comando de apoio: `python .gau/runtime/gau.py --project . hypotheses`.
   - Utilize os modelos JSON de `.gau/examples/` correspondentes.
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/hypothesis-decay.json`.
   - Registre: `mission`, `idea_id` (58), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `hypothesis-decay.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
