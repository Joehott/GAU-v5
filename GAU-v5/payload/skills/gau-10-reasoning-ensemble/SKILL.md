---
name: gau-10-reasoning-ensemble
description: Comparar linhas de solução independentes antes da síntese. Integra a ideia 10 do GAU v5.
---
...
- **Gatilho**: Comparar linhas de solução independentes antes da síntese.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `ensemble`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/ensemble.json`.
   - Registre: `mission`, `idea_id` (10), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `ensemble.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
