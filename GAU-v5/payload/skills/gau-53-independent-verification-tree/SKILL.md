---
name: gau-53-independent-verification-tree
description: Distribuir verificação separadamente de quem implementou em tarefa crítica. Integra a ideia 53 do GAU v5.
---

# Independent Verification Tree

## 1. Entrada e Ativação

- **Gatilho**: Distribuir verificação separadamente de quem implementou em tarefa crítica.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `independent-tree`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/independent-tree.json`.
   - Registre: `mission`, `idea_id` (53), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `independent-tree.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
