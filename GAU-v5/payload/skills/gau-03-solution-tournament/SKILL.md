---
name: gau-03-solution-tournament
description: Comparar implementações concorrentes com o mesmo conjunto de testes. Integra a ideia 3 do GAU v5.
---
...
- **Gatilho**: Comparar implementações concorrentes com o mesmo conjunto de testes.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `tournament`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/tournament.json`.
   - Registre: `mission`, `idea_id` (3), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `tournament.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
