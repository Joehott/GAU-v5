---
name: gau-23-loop-breaker-council
description: Detectar tentativas repetidas sem informação nova e mudar a estratégia. Integra a ideia 23 do GAU v5.
---

# Loop Breaker Council

## 1. Entrada e Ativação

- **Gatilho**: Detectar tentativas repetidas sem informação nova e mudar a estratégia.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `loop-break`.
...
2. **Apoio Executável**:
   - Utilize o comando de apoio: `python .gau/runtime/gau.py --project . novelty`.
   - Utilize os modelos JSON de `.gau/examples/` correspondentes.
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/loop-break.json`.
   - Registre: `mission`, `idea_id` (23), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `loop-break.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
