---
name: gau-25-security-council
description: Auditar riscos concretos de autenticação, entrada, segredos, dependências, dados e runtime. Integra a ideia 25 do GAU v5.
---
...
- **Gatilho**: Auditar riscos concretos de autenticação, entrada, segredos, dependências, dados e runtime.
...
Consulte `.gau/docs/OPERACAO.md` para sintaxe dos comandos e `.gau/docs/CONTRATOS.md` para o contrato `security`.
...
3. **Geração do Artefato Verificável**:
   - Produza o artefato em `.gau/missions/<mission_id>/security.json`.
   - Registre: `mission`, `idea_id` (25), `status` (VERIFIED/FAILED/BLOCKED/SERIAL_LIMITED), `source`, `scope`, `evidence`, `findings`, `unknowns` e `next_actions`.
...
- Entregue o artefato `security.json`, os IDs de evidências registrados e o relatório ao coordenador.
- Condições de parada:
